...
static lv_disp_drv_t * monitor_drv;
static void (*monitor_ori_cb)(lv_disp_drv_t * disp_drv, uint32_t time, uint32_t px);
static uint32_t disp_ori_timer_period;
static uint32_t anim_ori_timer_period;
static lv_obj_t * home_btn;
static lv_timer_t * report_timer;

LV_IMG_DECLARE(img_home);

extern void lv_demo_app_return_home(void);
...
static void generate_report_async(void * data);

static void home_event_cb(lv_event_t * e)
{
    LV_UNUSED(e);
    lv_demo_app_return_home();
}
...
static void benchmark_init(void)
{
    ...
    monitor_drv = disp->driver;
    monitor_ori_cb = monitor_drv->monitor_cb;
    disp->driver->monitor_cb = monitor_cb;

    scene_act = -1;
    opa_mode = true;
    for(uint32_t i = 0; scenes[i].create_cb; i++) {
        scenes[i].time_sum_normal = 0;
        scenes[i].time_sum_opa = 0;
        scenes[i].refr_cnt_normal = 0;
        scenes[i].refr_cnt_opa = 0;
        scenes[i].fps_normal = 0;
        scenes[i].fps_opa = 0;
    }
    ...
    home_btn = lv_img_create(lv_layer_top());
    lv_img_set_src(home_btn, &img_home);
    lv_obj_add_flag(home_btn, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_align(home_btn, LV_ALIGN_TOP_RIGHT, -3, 3);
    lv_obj_add_event_cb(home_btn, home_event_cb, LV_EVENT_CLICKED, NULL);
...
{
    lv_async_call_cancel(generate_report_async, NULL);

    if(next_scene_timer) lv_timer_del(next_scene_timer);
    next_scene_timer = NULL;

    if(report_timer) lv_timer_del(report_timer);
    report_timer = NULL;
    ...
    if(monitor_drv) {
        monitor_drv->monitor_cb = monitor_ori_cb;
        monitor_drv = NULL;
    }
    ...
    /*The report screen uses a flex layout, restore the screen so the next UI is not affected*/
    lv_obj_set_layout(lv_scr_act(), 0);         /*0 = no layout*/
...
void lv_demo_benchmark_run_scene(int_fast16_t scene_no)
{
    ...
    if(scenes[scene_act].create_cb) {
        ...
        report_timer = lv_timer_create(report_cb, SCENE_TIME, NULL);
        lv_timer_set_repeat_count(report_timer, 1);
    }
}
...
static void monitor_cb(lv_disp_drv_t * drv, uint32_t time, uint32_t px)
{
    LV_UNUSED(drv);
    LV_UNUSED(px);
    if(scene_act < 0 || !scenes[scene_act].create_cb) return;
...
static void generate_report(void)
{
    ...
    uint32_t opa_speed_pct = fps_normal_unweighted ?
                             (fps_opa_unweighted * 100) / fps_normal_unweighted : 0;
    ...
    lv_table_set_col_width(table, 0, (w * 3) / 5);
    lv_table_set_col_width(table, 1, w / 5);
    lv_table_set_col_width(table, 2, w / 5);
    ...
    lv_table_set_cell_value(table, 0, 0, "Test");
    lv_table_set_cell_value(table, 0, 1, "FPS");
    lv_table_set_cell_value(table, 0, 2, "Opa FPS");

    char buf[16];
    for(i = 0; scenes[i].create_cb; i++) {
        lv_table_set_cell_value(table, i + 1, 0, scenes[i].name);
...
static void report_cb(lv_timer_t * timer)
{
    if(timer == report_timer) report_timer = NULL;
...
static void next_scene_timer_cb(lv_timer_t * timer)
{
    ...
    /*Ready*/
        ...
        lv_async_call(generate_report_async, NULL);
    }
}
