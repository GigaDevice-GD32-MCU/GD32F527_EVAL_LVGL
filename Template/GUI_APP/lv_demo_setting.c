...
/* Use the complete Chinese font already included by the project. */
...
static const char str_mcu_details_cn[] = {0xE5,0x88,0xB6,0xE9,0x80,0xA0,0xE5,0x95,0x86,0x3A,0x20,0xE5,0x85,0x86,0xE6,0x98,0x93,0xE5,0x88,0x9B,0xE6,0x96,0xB0,0x0D,0x0A,0x4D,0x43,0x55,0xE5,0x9E,0x8B,0xE5,0x8F,0xB7,0x3A,0x20,0x47,0x44,0x33,0x32,0x46,0x35,0x32,0x37,0x49,0x4D,0x0D,0x0A,0x52,0x41,0x4D,0xE5,0xA4,0xA7,0xE5,0xB0,0x8F,0x3A,0x20,0x35,0x31,0x32,0x4B,0x42,0x0D,0x0A,0x52,0x4F,0x4D,0xE5,0xA4,0xA7,0xE5,0xB0,0x8F,0x3A,0x20,0x37,0x36,0x38,0x30,0x4B,0x42,0x0D,0x0A,0xE5,0x9B,0xBA,0xE4,0xBB,0xB6,0xE7,0x89,0x88,0xE6,0x9C,0xAC,0x3A,0x20,0x31,0x2E,0x34,0x2E,0x30,0x00};
...
void update_main_page_language(void)
{
    ...
    if(label_widget != NULL) {
        ...
        lv_obj_set_style_text_font(label_widget, font, 0);
    ...
    if(label_music != NULL) {
        ...
        lv_obj_set_style_text_font(label_music, font, 0);
    ...
    if(label_speed != NULL) {
        ...
        lv_obj_set_style_text_font(label_speed, font, 0);
    ...
    if(label_picture != NULL) {
        ...
        lv_obj_set_style_text_font(label_picture, font, 0);
    ...
    if(label_about != NULL) {
        ...
        lv_obj_set_style_text_font(label_about, font, 0);
    ...
    if(label_qrcode != NULL) {
        ...
        lv_obj_set_style_text_font(label_qrcode, font, 0);
    ...
    if(label_file != NULL) {
        ...
        lv_obj_set_style_text_font(label_file, font, 0);
    ...
    if(label_setting != NULL) {
        ...
        lv_obj_set_style_text_font(label_setting, font, 0);
        lv_obj_align_to(label_setting, imgbtn_setting, LV_ALIGN_OUT_BOTTOM_MID,  0, 2);
    }
   
    
    if(label_total_size != NULL) {
        lv_obj_set_style_text_font(label_total_size, font, 0);
    ...
    if(label_mem_used != NULL) {
        ...
        lv_obj_set_style_text_font(label_mem_used, font, 0);
        lv_label_set_text_fmt(label_mem_used, "%s: %"LV_PRId32".%"LV_PRId32" KB", str_mem_used[current_language], used_kb, (used_size % 100));
    }
    if(label_frag != NULL) {
        lv_obj_set_style_text_font(label_frag, font, 0);
        lv_label_set_text_fmt(label_frag, "%s: %"LV_PRId32".%"LV_PRId32" KB", str_frag[current_language], g_mon.frag_pct*g_mon.total_size/1024/100, g_mon.frag_pct*g_mon.total_size/1024%100);
    }
    if(label_temp != NULL) {
...
void lv_demo_setting(lv_obj_t * parent)
{
    ...
    create_text(section, NULL, str_mcu_details[current_language], LV_MENU_ITEM_BUILDER_VARIANT_1);
    ...
    create_text(section, NULL, str_lvgl_version[current_language], LV_MENU_ITEM_BUILDER_VARIANT_1);
